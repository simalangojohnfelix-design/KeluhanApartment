...
<body class="bg-gray-50 flex h-screen overflow-hidden" style="font-family: 'Inter', sans-serif;">
    ...
    <aside class="w-64 bg-gray-950 text-white flex flex-col shadow-2xl flex-shrink-0">
        ...
        <nav class="flex-1 py-6 px-4 space-y-2 text-sm font-medium">
            ...
            <a href="<?php echo e(route('owner.approvals.index')); ?>" class="block px-4 py-2.5 rounded-lg text-gray-400 hover:text-white hover:bg-gray-900 transition">Persetujuan Biaya</a>
            ...
            <a href="<?php echo e(route('owner.assets.index')); ?>" class="block px-4 py-2.5 bg-blue-600 text-white font-bold shadow-md rounded-lg transition">Kelola Aset</a>
        </nav>
    ...
    <main class="flex-1 overflow-y-auto p-10">
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="<?php echo e(route('owner.assets.create')); ?>" class="bg-gray-900 hover:bg-black text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md transition flex items-center gap-2">
                    + Tambah Aset Baru
                </a>
            </div>
        </div>
        ...
        <div class="flex overflow-x-auto gap-8 pb-8 snap-x">
            <?php $__empty_1 = true; $__currentLoopData = $groupedAssets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $unitId => $unitAssets): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                    ...
                    <div class="flex justify-between items-center px-6 py-4 bg-gray-900 text-white">
                        <div>
                            <span class="font-bold text-xl font-playfair">
                                <?php echo e($unit ? 'Unit ' . $unit->unit_number : 'Fasilitas Umum'); ?>

                            </span>
                            <?php if($unit && $unit->type): ?>
                                <span class="ml-2 text-sm text-gray-400">Tipe: <?php echo e($unit->type); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="text-right text-sm">
                            <span class="text-gray-300">Penyewa:</span> 
                            <span class="font-bold"><?php echo e($unit && optional($unit->user)->name ? $unit->user->name : 'Kosong / Umum'); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="overflow-y-auto flex-1 max-h-[60vh]">
                        <table class="min-w-full text-left">
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                <?php $__currentLoopData = $unitAssets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $asset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-6 py-4 font-bold text-gray-900"><?php echo e($asset->name); ?></td>
                                    <td class="px-6 py-4 text-gray-700"><?php echo e($asset->type); ?></td>
                                    <td class="px-6 py-4">
                                        <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider
                                            <?php echo e($asset->status == 'good' ? 'bg-green-100 text-green-700' : 
                                              ($asset->status == 'maintenance' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700')); ?>">
                                            <?php echo e($asset->status == 'good' ? 'Baik' : ($asset->status == 'damaged' ? 'Rusak' : 'Perawatan')); ?>

                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="<?php echo e(route('owner.assets.edit', $asset->id)); ?>" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-xs font-bold transition border border-gray-200">
                                                Edit
                                            </a>
                                            <form action="<?php echo e(route('owner.assets.destroy', $asset->id)); ?>" method="POST" onsubmit="return confirm('Hapus data aset inventaris ini?');">
...
</html><?php /**PATH D:\MSI\sistem-apartemen\resources\views/owner/assets/index.blade.php ENDPATH**/ ?>