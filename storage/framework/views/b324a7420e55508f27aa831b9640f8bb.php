...
<div class="max-w-4xl mx-auto space-y-6" style="font-family: 'Inter', sans-serif;">
    ...
    <div class="flex justify-between items-center">
        <a href="<?php echo e(route('technician.tasks.index')); ?>" class="text-sm font-medium text-gray-500 hover:text-blue-600 transition">&larr; Kembali ke Daftar Tugas</a> 
        
        <a href="<?php echo e(route('technician.tasks.invoice', $task->id)); ?>" target="_blank" class="bg-gray-900 hover:bg-black text-white px-5 py-2.5 rounded-xl font-bold transition text-sm flex items-center gap-2 shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div class="flex justify-between items-start mb-6">
            <div>
                <span class="text-xs font-bold tracking-widest uppercase text-blue-600">Work Order #WO-<?php echo e($task->id); ?></span>
                <h2 class="text-2xl font-bold text-gray-800 mt-1" style="font-family: 'Playfair Display', serif;"><?php echo e(optional($task->complaint)->title); ?></h2>
            </div>
            <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider 
                <?php echo e($task->status == 'completed' ? 'bg-green-100 text-green-700' : ($task->status == 'waiting_approval' ? 'bg-amber-100 text-amber-700' : ($task->status == 'in_progress' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'))); ?>">
                Status: <?php echo e(str_replace('_', ' ', $task->status)); ?>

            </span>
        </div>

        <p class="text-sm text-gray-500 mb-4">
            Unit: <strong class="text-gray-800">Unit <?php echo e(optional(optional($task->complaint)->propertyUnit)->unit_number); ?></strong> &bull; 
            Penyewa: <strong class="text-gray-800"><?php echo e(optional(optional($task->complaint)->tenant)->name); ?></strong>
        </p>
        <p class="text-gray-700 bg-gray-50 p-4 rounded-xl border border-gray-100 text-sm leading-relaxed mb-6"><?php echo e(optional($task->complaint)->description); ?></p>
        ...
        <div class="mb-8">
            ...
            <img src="<?php echo e(asset('storage/'.$task->complaint->photo)); ?>" class="rounded-xl max-h-56 object-cover border border-gray-200 shadow-sm" alt="Foto Keluhan">
        </div>
        <?php endif; ?>

        <form action="<?php echo e(route('technician.tasks.update', $task->id)); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?> 

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Detail Tindakan Perbaikan Lapangan</label>
                <textarea name="action_details" rows="3" class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-blue-500 outline-none" placeholder="Jelaskan tindakan perbaikan..." <?php echo e(in_array($task->status, ['waiting_approval', 'completed']) ? 'readonly' : ''); ?>><?php echo e($task->action_details); ?></textarea>
            </div>
            ...
            <div>
                <div class="flex justify-between items-center mb-3">
                    ...
                    <?php if(!in_array($task->status, ['waiting_approval', 'completed'])): ?>
                            ...
                            <tr>
                                ...
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        <input type="number" id="estimatedDays" name="estimated_days" value="<?php echo e($task->estimated_days ?? 1); ?>" min="1" class="w-16 border border-gray-300 rounded-lg p-1.5 text-center text-sm outline-none bg-white" oninput="calculateTotal()" <?php echo e(in_array($task->status, ['waiting_approval', 'completed']) ? 'readonly' : ''); ?>>
                                        <span class="text-xs text-gray-500">Hari</span>
                                    </div>
                                </td>
                                <td class="p-3 text-right font-medium text-gray-800" colspan="2">
                                    <span id="laborTotalDisplay">Rp <?php echo e(number_format(($task->estimated_days ?? 1) * 50000, 0, ',', '.')); ?></span>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot class="bg-gray-50 border-t-2 border-gray-200 font-bold text-gray-900">
                            <tr>
                                ...
                                <td colspan="2" class="p-3 text-right text-base text-blue-600">
                                    ...
                                    <input type="hidden" id="costEstimate" name="cost_estimate" value="<?php echo e($task->cost_estimate ?? 0); ?>">
                                    <input type="hidden" id="sparePartsJson" name="spare_parts" value="<?php echo e($task->spare_parts); ?>">
                                </td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                <div>
                    ...
                    <?php endif; ?>
                    <?php if($task->photo_before): ?>
                        <img src="<?php echo e(asset('storage/'.$task->photo_before)); ?>" class="mt-2 rounded-xl max-h-32 object-cover border shadow-sm">
                    <?php endif; ?>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">
                        Foto Kondisi SESUDAH 
                        <?php if($task->status == 'in_progress'): ?> <span class="text-red-500 font-normal">(Wajib untuk menyelesaikan)</span> <?php endif; ?>
                    </label>
                    <?php if($task->status != 'completed'): ?>
                        <input type="file" name="photo_after" accept="image/*" class="w-full text-sm text-gray-500" <?php echo e($task->status == 'in_progress' ? 'required' : ''); ?>>
                    <?php endif; ?>
                    <?php if($task->photo_after): ?>
                        <img src="<?php echo e(asset('storage/'.$task->photo_after)); ?>" class="mt-2 rounded-xl max-h-32 object-cover border shadow-sm">
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-gray-200 flex justify-end">
                <?php if($task->status == 'waiting_approval'): ?>
...
const isLocked = <?php echo e(in_array($task->status, ['waiting_approval', 'completed']) ? 'true' : 'false'); ?>;
