...
<body class="bg-gray-50 flex h-screen overflow-hidden" style="font-family: 'Inter', sans-serif;">
    ...
    <main class="flex-1 overflow-y-auto p-10">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-2xl">
            <form action="<?php echo e(route('owner.units.update', $unit->id)); ?>" method="POST" class="space-y-6">
                <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                <div><label class="block text-sm font-bold text-gray-700 mb-2">Nomor Unit</label><input type="text" name="unit_number" value="<?php echo e($unit->unit_number); ?>" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" required></div>
                <div><label class="block text-sm font-bold text-gray-700 mb-2">Tipe Unit</label><input type="text" name="type" value="<?php echo e($unit->type); ?>" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" required></div>
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="available" <?php echo e($unit->status == 'available' ? 'selected' : ''); ?>>Tersedia (Available)</option>
                        <option value="occupied" <?php echo e($unit->status == 'occupied' ? 'selected' : ''); ?>>Terisi (Occupied)</option>
                        <option value="maintenance" <?php echo e($unit->status == 'maintenance' ? 'selected' : ''); ?>>Dalam Perawatan (Maintenance)</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="user_id" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <?php $__currentLoopData = $tenants; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tenant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($tenant->id); ?>" <?php echo e($unit->user_id == $tenant->id ? 'selected' : ''); ?>><?php echo e($tenant->name); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="pt-4 flex gap-4">
                    <a href="<?php echo e(route('owner.units.index')); ?>" class="px-6 py-3 bg-gray-100 text-gray-700 font-bold rounded-xl text-sm hover:bg-gray-200 transition">Batal</a>
...
</html><?php /**PATH D:\MSI\sistem-apartemen\resources\views/owner/units/edit.blade.php ENDPATH**/ ?>