...
<body class="bg-gray-50 flex h-screen overflow-hidden" style="font-family: 'Inter', sans-serif;">
    <aside class="w-64 bg-gray-950 text-white flex flex-col shadow-2xl flex-shrink-0">
        ...
        <nav class="flex-1 py-6 px-4 space-y-2 text-sm font-medium">
            ...
            <a href="<?php echo e(route('owner.users.index')); ?>" class="block px-4 py-2.5 bg-blue-600 text-white font-bold shadow-md rounded-lg transition">Kelola Pengguna</a>
    ...
    <main class="flex-1 overflow-y-auto p-10">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-2xl">
            <form action="<?php echo e(route('owner.users.update', $user->id)); ?>" method="POST" class="space-y-6">
                <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                <div><label class="block text-sm font-bold text-gray-700 mb-2">Nama Lengkap</label><input type="text" name="name" value="<?php echo e($user->name); ?>" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" required></div>
                <div><label class="block text-sm font-bold text-gray-700 mb-2">Email</label><input type="email" name="email" value="<?php echo e($user->email); ?>" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" required></div>
                ...
                <div><label class="block text-sm font-bold text-gray-700 mb-2">No. Telepon</label><input type="text" name="phone" value="<?php echo e($user->phone); ?>" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500"></div>
                <div>
                    ...
                    <select name="role" class="w-full border border-gray-300 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="tenant" <?php echo e($user->role == 'tenant' ? 'selected' : ''); ?>>Tenant (Penyewa)</option>
                        <option value="technician" <?php echo e($user->role == 'technician' ? 'selected' : ''); ?>>Technician (Teknisi)</option>
                        <option value="admin" <?php echo e($user->role == 'admin' ? 'selected' : ''); ?>>Admin (Dispatcher)</option>
                        <option value="owner" <?php echo e($user->role == 'owner' ? 'selected' : ''); ?>>Owner (Pemilik)</option>
                    </select>
                </div>
                <div class="pt-4 flex gap-4">
                    <a href="<?php echo e(route('owner.users.index')); ?>" class="px-6 py-3 bg-gray-100 text-gray-700 font-bold rounded-xl text-sm hover:bg-gray-200 transition">Batal</a>
...
</html><?php /**PATH D:\MSI\sistem-apartemen\resources\views/owner/users/edit.blade.php ENDPATH**/ ?>