...
<head>
    ...
    <title>Invoice #INV-WO-{{ $task->id }} - Palazzo Palace</title>
...
<body class="bg-gray-100 py-6 px-4 text-xs" style="font-family: 'Inter', sans-serif;" onload="window.print()">
    ...
    <div class="max-w-2xl mx-auto mb-4 flex justify-between items-center no-print">
        <a href="{{ route('technician.tasks.show', $task->id) }}" class="text-xs font-bold text-gray-600 hover:text-gray-900 bg-white px-3 py-1.5 rounded-lg shadow-sm border border-gray-200 transition">
    ...
    <div class="invoice-container max-w-2xl mx-auto bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        ...
        <div class="flex justify-between items-start border-b border-gray-200 pb-4 mb-4">
            ...
            <div class="text-right">
                ...
                <p class="text-xs font-bold text-gray-800">No: INV/WO/{{ date('Y') }}/00{{ $task->id }}</p>
                <p class="text-[10px] text-gray-500">Tanggal: {{ date('d/m/Y') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4 bg-gray-50 p-3.5 rounded-lg border border-gray-100">
            <div>
                ...
                <p class="text-gray-600 text-[11px]">Unit Perbaikan: <strong class="text-gray-800">No. {{ optional(optional($task->complaint)->propertyUnit)->unit_number }}</strong> (Tenant: {{ optional(optional($task->complaint)->tenant)->name }})</p>
            </div>
            <div>
                <p class="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Informasi Penugasan:</p>
                <p class="text-gray-700 text-[11px]">Teknisi: <strong class="text-gray-900">{{ optional($task->technician)->name }}</strong></p>
                <p class="text-gray-700 text-[11px]">Status: <strong class="text-blue-600 uppercase text-[10px]">{{ str_replace('_', ' ', $task->status) }}</strong></p>
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="border border-gray-200 rounded-lg p-3 bg-white">
                <p class="font-bold text-gray-900 text-[11px]">{{ optional($task->complaint)->title }}</p>
                <p class="text-[11px] text-gray-600 mt-0.5 leading-relaxed">{{ optional($task->complaint)->description }}</p>
            </div>
        </div>

        @if($task->action_details)
        <div class="mb-4">
            ...
            <div class="border border-gray-200 rounded-lg p-3 bg-gray-50/50">
                <p class="text-[11px] text-gray-700 leading-relaxed">{{ $task->action_details }}</p>
            </div>
        </div>
        @endif
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-200 text-[11px]">
                        ...
                        @if(is_array($items) && count($items) > 0)
                            ...
                            <tr>
                                <td class="p-2.5 font-medium text-gray-800">{{ $item['name'] ?: 'Material Perbaikan' }}</td>
                                <td class="p-2.5 text-center text-gray-600">{{ $item['qty'] ?? '1 Pcs' }}</td>
                                <td class="p-2.5 text-right font-medium text-gray-800">Rp {{ number_format($item['price'] ?? 0, 0, ',', '.') }}</td>
                            </tr>
                            @endforeach
                        @else
                            <tr>
                        ...
                        <tr class="bg-gray-50/50">
                            ...
                            <td class="p-2.5 text-center text-gray-600">{{ $task->estimated_days ?? 1 }} Hari</td>
                            <td class="p-2.5 text-right font-medium text-gray-800">
                                Rp {{ number_format(($task->estimated_days ?? 1) * 50000, 0, ',', '.') }}
                            </td>
                        </tr>
                    </tbody>
                    <tfoot class="bg-gray-100 border-t-2 border-gray-200 font-bold text-gray-900">
                        <tr>
                            ...
                            <td class="p-2.5 text-right text-sm text-blue-600">
                                Rp {{ number_format($task->cost_estimate ?? 0, 0, ',', '.') }}
                            </td>
                        </tr>
                    </tfoot>
                </table>
