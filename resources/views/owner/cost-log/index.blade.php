...
<body class="bg-gray-50 flex h-screen overflow-hidden" style="font-family: 'Inter', sans-serif;">
    ...
    <aside class="w-64 bg-gray-950 text-white flex flex-col shadow-2xl flex-shrink-0">
        ...
        <nav class="flex-1 py-6 px-4 space-y-2 text-sm font-medium">
            ...
            <a href="{{ route('owner.cost-log.index') }}" class="block px-4 py-2.5 bg-blue-600 text-white font-bold shadow-md rounded-lg transition">Riwayat Biaya (Log)</a>
    ...
    <main class="flex-1 overflow-y-auto p-10">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div class="bg-white px-6 py-4 rounded-2xl border border-gray-200 shadow-sm min-w-[250px] text-right">
                ...
                <p class="text-3xl font-bold text-blue-600">Rp {{ number_format($totalCost, 0, ',', '.') }}</p>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="p-5">
                                <span class="font-bold text-gray-800 block">WO-{{ $log->id }}</span>
                                <span class="text-xs text-gray-500 font-medium">Unit: {{ optional(optional($log->complaint)->propertyUnit)->unit_number ?? '-' }}</span>
                            </td>
                            <td class="p-5 font-medium text-gray-700">
                                {{ optional($log->technician)->name ?? '-' }}
                            </td>
                            <td class="p-5 text-gray-600">
                                <div class="line-clamp-2 leading-relaxed" title="{{ $log->action_details }}">{{ $log->action_details ?? '-' }}</div>
                            </td>
