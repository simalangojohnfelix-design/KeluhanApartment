...
<body class="bg-gray-50 flex h-screen overflow-hidden" style="font-family: 'Inter', sans-serif;">
    ...
    <main class="flex-1 overflow-y-auto p-10">
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <a href="{{ route('owner.units.create') }}" class="bg-gray-900 hover:bg-black text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md transition flex items-center gap-2">
                + Tambah Unit Baru
            </a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="p-5 font-bold text-gray-900">{{ $unit->unit_number }}</td>
                        <td class="p-5 text-gray-700">{{ $unit->type }}</td>
                        <td class="p-5">
                        ...
                        <td class="p-5 font-medium text-gray-800">{{ optional($unit->user)->name ?? 'Kosong (Belum Disewa)' }}</td>
                        <td class="p-5">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('owner.units.edit', $unit->id) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-xs font-bold transition border border-gray-200">Edit</a>
                                <form action="{{ route('owner.units.destroy', $unit->id) }}" method="POST" onsubmit="return confirm('Hapus data unit ini secara permanen?');">
                                    @csrf @method('DELETE')
