...
<div class="mb-10 flex flex-col md:flex-row md:items-center justify-between border-b border-gray-200 pb-6" data-aos="fade-down">
    <div>
        <a href="{{ route('tenant.dashboard') }}" class="text-sm font-medium text-gray-500 hover:text-blue-600 transition mb-2 inline-block" style="font-family: 'Inter', sans-serif;">&larr; Batal & Kembali ke Dasbor</a>
    ...
    <a href="{{ route('tenant.complaints.create') }}" class="mt-4 md:mt-0 bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-md inline-flex items-center">
        + Buat Laporan Baru
    </a>
</div>
...
<div class="space-y-4" style="font-family: 'Inter', sans-serif;">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row md:items-center justify-between group hover:shadow-md transition-all duration-300" data-aos="fade-up" data-aos-delay="{{ $index * 50 }}">
        <div class="flex-1">
            <div class="flex items-center gap-3 mb-3">
                <span class="text-xs font-bold tracking-widest uppercase text-gray-500">{{ $c->category }}</span>
                <span class="w-1.5 h-1.5 rounded-full bg-gray-300"></span>
                <!-- Indikator Warna Urgensi -->
                <span class="text-xs font-bold tracking-widest uppercase 
                    {{ $c->urgency == 'high' ? 'text-red-600' : ($c->urgency == 'medium' ? 'text-yellow-600' : 'text-green-600') }}">
                    Urgensi: {{ ucfirst($c->urgency) }}
                </span>
            </div>
            <h3 class="text-xl font-bold text-gray-800 mb-2">{{ $c->title }}</h3>
            <p class="text-gray-500 text-sm line-clamp-2">{{ Str::limit($c->description, 100) }}</p>
        </div>
        ...
        <div class="mt-4 md:mt-0 md:ml-8 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4">
            ...
            <a href="{{ route('tenant.complaints.show', $c->id) }}" class="text-sm font-bold text-blue-600 hover:text-blue-800 transition">
                Lihat Detail &rarr;
            </a>
        </div>
    </div>
