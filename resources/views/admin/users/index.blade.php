...
<body class="bg-gray-50 flex h-screen overflow-hidden" style="font-family: 'Inter', sans-serif;">
    ...
    <aside class="w-64 bg-gray-950 text-white flex flex-col shadow-2xl flex-shrink-0">
        ...
        <nav class="flex-1 py-6 px-4 space-y-2">
            ...
            <a href="{{ route('admin.users.index') }}" class="block px-4 py-3 bg-blue-600 text-white rounded-lg font-bold shadow-md transition">Daftar Pengguna</a>
        </nav>
    ...
    <main class="flex-1 overflow-y-auto p-10">
        ...
        <!-- Tabel Tenants -->
        <div class="mb-10">
            <h3 class="text-lg font-bold text-gray-800 mb-4 border-l-4 border-blue-600 pl-3">Penyewa (Tenant) - {{ $tenants->count() }} orang</h3>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="p-4">
                                <div class="text-sm font-medium text-gray-800">{{ $t->email }}</div>
                                <div class="text-xs text-gray-500 mt-1">{{ $t->phone ?: 'Tidak ada nomor' }}</div>
                            </td>
                            <td class="p-4">
                                @if($t->propertyUnit)
                                    <span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-bold">{{ $t->propertyUnit->unit_number }}</span>
                                    <span class="text-xs text-gray-500 ml-2 font-medium">Lantai {{ $t->propertyUnit->floor }}</span>
                                @else
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
            <div>
                <h3 class="text-lg font-bold text-gray-800 mb-4 border-l-4 border-green-600 pl-3">Tim Teknisi - {{ $technicians->count() }} orang</h3>
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-gray-50 border-b border-gray-200 text-xs text-gray-600 uppercase tracking-wide">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="p-4 font-bold text-gray-800">{{ $t->name }}</td>
                                <td class="p-4 text-sm text-gray-600 font-medium">{{ $t->email }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="p-4 font-bold text-gray-800">
                                    {{ $a->name }}
                                    <div class="text-xs text-gray-500 font-medium mt-1">{{ $a->email }}</div>
                                </td>
                                <td class="p-4">
                                    <span class="bg-gray-900 text-white px-3 py-1 rounded-full text-xs uppercase font-bold tracking-wider">{{ $a->role }}</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
